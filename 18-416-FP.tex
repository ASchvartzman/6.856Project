...
Here we present Poloczek and Shcnitger's algorithm. In this section,
...
\begin{eqnarray}
E[\textrm{sat}(i)] &=& p_0(\mathsf{fanout} + 2w_{\bar{x_i}}) + p_1(\mathsf{fanin} + 2w_{x_i})\\
E[\textrm{unsat}(i)] &=& p_0w_{x_i} + p_1w_{\bar{x_i}}
\end{eqnarray}
...
$E[\textrm{sat}(i) - 3\cdot\textrm{unsat}(i)] = E[\textrm{wounded} + \mathsf{slack}] - (w_{x_i} + w_{\bar{x_i}})$
\end{lemma}
Now consider some optimal assignment, $\pi$. Let us define $c(x_i)$ to be the weight of alive clauses that are not satisfied by $\pi$ before assigning a value to $x_i$, and $c'(x_i)$ be that same quantity after we assign a value to $x_i$. By linearity of expectations, we can add the quantity $2(c-c')$ to both sides of the equation of lemma \ref{L:2}, yielding:
\begin{equation}
E[\textrm{sat}(i) - 3\cdot\textrm{unsat}(i)+ 2(c-c')] = E[\textrm{wounded} + \mathsf{slack} + 2(c-c')] - (w_{x_i} + w_{\bar{x_i}})
\end{equation}
...
point that the term $\mathbb{E}(c'-c)$ is equal to
...
\[E[\textrm{wounded} - 2(c'-c)] - (w_{x_i} - w_{\bar{x_i}}) \ge \frac{\mathsf{slack}}{\Delta}(w_{x_i} - w_{\bar{x_i}}) + \varepsilon(\mathsf{fanin} + \mathsf{fanout})\]
...
\begin{eqnarray*}
E[\mathsf{slack}] &=& (p_1-p_0)\cdot ((\mathsf{fanout} + 2w_{\bar{x_i}}) -(\mathsf{fanin} + 2w_{x_i}))
...
\end{eqnarray*}
Plugging this into equation \ref{C:1}, and applying \ref{L:3}, we get that if $x_i =1$ is the optimal assignment,
\[E[\textrm{sat}(i) - 3\cdot\textrm{unsat}(i)+ 2(c-c')] \ge \frac{\mathsf{slack}^2}{\Delta} +2\varepsilon\cdot\mathsf{slack} - \frac{\mathsf{slack}}{\Delta}(w_{x_i} + w_{\bar{x_i}}) + \varepsilon(\mathsf{fanin} + \mathsf{fanout}).\]
