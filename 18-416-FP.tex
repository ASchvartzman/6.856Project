...
We will present a potential function $\phi$ such that it meets the criterion before. Our potential function will always
 ...
 $\phi(0) = 2(W-\mathrm{OPT})$. It is clear that the first two conditions will be met, so we will focus on showing \ref{eq:2}.
...
\begin{aligned}
\label{eq:4}
& \mathrm{SAT}(i) - \mathrm{SAT}(i-1) - 3(\mathrm{UNSAT}(i) - \mathrm{UNSAT}(i-1)) \\
...
In \cite{DBLP:conf/esa/Poloczek11},
Poloczek showed not only
that the randomized algorithms of the previous two sections
gave the best possible approximation of any algorithm using the same approach
but also that the randomness was necessary that performance.
